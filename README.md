...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/tax_regulation \
  -H 'Content-Type: application/json' \
  -d '{"citation":"1.170A-1"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/tax_regulation`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "tax-regulations": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-tax-regulations"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-tax-regulations
```
